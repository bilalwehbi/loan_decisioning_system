# src/models/test_fraud_detection_inference.py
import os
import sys
import logging
import json
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('fraud_inference_test.log')
    ]
)
...
def load_latest_model():
    ...
    if not model_files:
        raise FileNotFoundError("No trained fraud detection models found. Run training first.")
...
# src/models/test_fraud_detection_inference.py (update the create_sample_applications function)
...
def create_sample_applications(num_samples=5):
    ...
    # Select random samples, including at least one known fraud
    try:
        fraud_samples = df[df['fraud_flag'] == 1].sample(min(2, len(df[df['fraud_flag'] == 1])))
        normal_samples = df[df['fraud_flag'] == 0].sample(num_samples - len(fraud_samples))
        samples = pd.concat([fraud_samples, normal_samples])
    except:
        # Fallback if fraud_flag column doesn't exist or has issues
    ...
    for _, row in samples.iterrows():
        # Create a cleaner, more comprehensive application
        application = {
            ...
            'credit_data': {
                'credit_score': int(row['credit_score']) if pd.notna(row['credit_score']) else 650,
        ...
        if 'loan_status' in row:
            actual_outcome['actual_loan_status'] = row['loan_status']
        if 'fraud_flag' in row:
            actual_outcome['actual_fraud'] = int(row['fraud_flag'])
...
def test_inference():
    ...
    for i, (application, actual) in enumerate(applications, 1):
        ...
        # Log results
        logger.info(f"Prediction: Fraud Score: {prediction['fraud_score']:.4f}, Actual Fraud: {actual['actual_fraud']}")
        logger.info(f"Fraud Flags: {', '.join(prediction['fraud_flags']) if prediction['fraud_flags'] else 'None'}")
        logger.info("---")

if __name__ == "__main__":
    logger.info("Testing fraud detection model inference")
    test_inference()
